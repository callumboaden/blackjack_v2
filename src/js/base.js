
const elements = {
    dealButton: document.querySelector('.button__deal'),
    dealerHandDisplay: document.querySelector('.dealer__hand'),
    playerHandListDisplay: document.querySelector('.player__hand__list'),
    playerBankDisplay: document.querySelector('.player__bank__display__amount'),
    playerBetDisplay: document.querySelector('.player__bet__display__amount'),
    playerWinDisplay: document.querySelector('.player__win__display__amount'),
    playButtons: document.querySelector('.buttons__play'),
    betButtons: document.querySelector('.buttons__bet'),
}